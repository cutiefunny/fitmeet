...
<div class="admin-container">
	<div class="header-area">
		...
		{:else}
			<button class="toggle-btn secondary" on:click={toggleMode} disabled={isUploading}
				>← 목록으로</button
			>
		{/if}
	</div>
	...
	{#if isFormMode}
		<div class="form-container fade-in">
			...
			<form on:submit|preventDefault={handleSubmit} class="member-form">
				...
				<div class="form-group">
					<label for="likeCount">남은 LIKE 횟수</label>
					<input type="number" id="likeCount" bind:value={likeCount} required />
				</div>
						...
						<div class="member-item">
							<div class="member-content">
								...
								<div class="member-info">
									<h3>
										{member.name}
										<span class="member-age">({member.age}, {member.gender})</span>
									</h3>
									...
									<p class="member-likes">
										남은 LIKE: <span>{member.likeCount ?? 0}</span>
									</p>
								</div>
							</div>

							<div class="member-actions">
								<button class="action-btn edit-btn" on:click={() => handleEdit(member)}
									>수정</button
								>
								<button class="action-btn delete-btn" on:click={() => handleDelete(member.id)}
									>삭제</button
								>
							</div>
						</div>
